...
import type { ContinueConversationResponse } from '../responses';

export type ContinueConversationInput = Readonly<{
  conversationId: string;
  messageCount: number;
}>;

export class ContinueConversationUseCase extends UseCase<
  ContinueConversationInput,
  ContinueConversationResponse
> {
  public readonly name = 'ContinueConversation';
  ...
  public async execute(
  ...
  ): Promise<Result<ContinueConversationResponse, ApplicationError>> {
    this.logger.info(`Executing ${this.name}`, {
      conversationId: input.conversationId,
    });

    // 1. validate
    if (!input.conversationId || typeof input.messageCount !== 'number') {
    ...
    const outcome = conversation.continue(
      input.messageCount,
