...
import { CompleteConversationUseCase } from './complete-conversation-use-case';
...
describe('CompleteConversationUseCase', () => {
  let useCase: CompleteConversationUseCase;
  ...
  beforeEach(() => {
    ...
    useCase = new CompleteConversationUseCase(
      mockPort,
      mockPublisher,
      mockClock,
      mockUuid,
  ...
  it('should complete conversation and save it', async () => {
  ...
  it('should return idempotent if already completed', async () => {
    const existing = Conversation.rehydrate(
      'test-id' as Uuid,
      {
        status: 'qualified',
